...
$G$ tiene un camino hamiltoniano si y solo si existe un árbol generador $T$ de $G$ tal que $deg_T(v) \le 2$ para todo $v \in V$.

\textit{($\Rightarrow$)} Si $G$ tiene un camino hamiltoniano $P$ que visita todos los vértices exactamente una vez,
entonces las aristas de $P$ forman un subgrafo conexo con $|V|-1$ aristas, por lo que es un árbol generador $T$.
Además, en un camino los extremos tienen grado $1$ y los vértices internos grado $2$, así que $\deg_T(v)\le 2$ para todo $v$.

\textit{($\Leftarrow$)} Supongamos ahora que existe un árbol generador $T$ de $G$ tal que $\deg_T(v)\le 2$ para todo $v$.
Un árbol en el que todos los vértices tienen grado a lo sumo $2$ necesariamente es un camino simple que incluye a todos los vértices:
si existiera un vértice de grado $3$ o más no cumpliría la cota; y si tuviera varios componentes no sería árbol generador.
Por lo tanto, $T$ es un camino que visita todos los vértices exactamente una vez, es decir, un camino hamiltoniano en $G$.
\end{proof}
...
\subsection{Enumeración Exhaustiva con Gosper's hack}

\subsubsection{Descripción del algoritmo}
Sea $E$ el conjunto de aristas del grafo, construido tomando todos los pares $i<j$ con peso $\texttt{mat}[i][j]$.
El algoritmo enumera \textbf{todos} los subconjuntos de aristas $E' \subseteq E$ con cardinalidad exactamente $|E'|=V-1$.
Para cada subconjunto:
\begin{enumerate}
    \item Construye el subgrafo inducido $G'=(V,E')$ mediante listas de adyacencia.
    \item Calcula el costo total $\sum_{e \in E'} w(e)$.
    \item Verifica factibilidad mediante \texttt{check(V,K,adj)}, que comprueba:
    \begin{itemize}
        \item que $G'$ sea un árbol generador (conexo y acíclico), y
        \item que $\deg_{G'}(v) \le K$ para todo $v\in V$.
    \end{itemize}
    \item Si es factible y su costo es menor que el mejor registrado, actualiza la solución óptima.
\end{enumerate}

La enumeración se realiza usando una \textbf{máscara de bits} \texttt{mask} de 64 bits que indica qué aristas de $E$ están seleccionadas.
Para iterar eficientemente por todas las máscaras con exactamente $V-1$ bits en 1, se utiliza la técnica conocida como
\textit{Gosper's hack}, que genera en tiempo $O(1)$ la siguiente combinación de tamaño fijo.

\[
\begin{aligned}
c &= mask \ \&\ (-mask) \\
r &= mask + c \\
mask' &= r\ \vert\ \left(\frac{(r \oplus mask) \gg 2}{c}\right).
\end{aligned}
\]
Intuitivamente, $c$ aísla el bit menos significativo encendido, $r$ produce el “acarreo” que mueve hacia la izquierda
el bloque adecuado de bits, y el último término recoloca los bits restantes en las posiciones menos significativas para
mantener el conteo de unos constante.

\subsubsection{Correctitud}
\begin{lemma}[Exhaustividad]
El algoritmo considera todos los subconjuntos $E' \subseteq E$ tales que $|E'|=V-1$.
\end{lemma}
\begin{proof}
La variable \texttt{mask} codifica subconjuntos de $E$. Se inicializa con $V-1$ bits activados y se actualiza usando
\textit{Gosper's hack}, que genera sin repetición todas las máscaras de longitud $|E|$ con exactamente $V-1$ bits en 1.
Por tanto, se recorren todas las combinaciones posibles de $V-1$ aristas.
\end{proof}

\begin{lemma}[Filtrado de factibilidad]
Una combinación $E'$ es aceptada si y solo si induce un árbol generador que respeta la cota de grado $\deg(v)\le K$.
\end{lemma}
\begin{proof}
La función \texttt{check(V,K,adj)} verifica precisamente (i) que el subgrafo sea un árbol generador y (ii) que se cumpla la cota de grado.
Por consiguiente, el algoritmo acepta exactamente las combinaciones factibles.
\end{proof}

\begin{theorem}[Optimalidad]
Si existe al menos una solución factible, el algoritmo devuelve una solución factible de costo mínimo.
\end{theorem}
\begin{proof}
Por el Lema de Exhaustividad, toda solución candidata con $V-1$ aristas es evaluada. Por el Lema de Factibilidad, únicamente las soluciones factibles pueden actualizar el óptimo.
El algoritmo mantiene el mínimo costo entre todas las soluciones factibles examinadas; luego, el resultado final es óptimo.
\end{proof}

\subsection{Complejidad}
Sea $n=V$ y $m=|E|=\frac{n(n-1)}{2}$.
El número de subconjuntos evaluados es:
\[
\binom{m}{n-1}.
\]
Para cada subconjunto, el programa recorre todas las $m$ aristas para construir $E'$ y calcular el costo, lo que cuesta $O(m)$,
más el costo de \texttt{check}, típicamente $O(n)$ al operar sobre un subgrafo con $n-1$ aristas.
Por tanto, una cota superior del tiempo total es:
\[
...
\multirow{2}{*}{\bf{Grupos}} 
  ...
  & \multicolumn{2}{c|}{\bf{comb/gosper}}
...
De acuerdo con la tabla de comparación de algoritmos (\texttt{bitmask}, \texttt{comb/gosper} y \texttt{gray\_recursive}) se observan los siguientes resultados:

\begin{itemize}
    \item \textbf{Cobertura de grupos:} 
    \texttt{bitmask} y \texttt{comb/gosper} funcionan correctamente únicamente en los grupos más pequeños (\texttt{maxN6} y \texttt{maxN9}). 
    ...
    Para \texttt{maxN9}, \texttt{bitmask} alcanza solo 2000 puntos, mientras que \texttt{comb/gosper} y \texttt{gray\_recursive} logran 3000 puntos, mostrando que \texttt{bitmask} pierde eficiencia en instancias medianas. 
    ...
    Sumando los puntos obtenidos por cada algoritmo, \texttt{gray\_recursive} alcanza la puntuación más alta total (5648 puntos), seguido por \texttt{comb/gosper} (5500) y \texttt{bitmask} (4500). Esto refleja que \texttt{gray\_recursive} es más robusto frente a instancias medianas, aunque sigue limitado en instancias grandes.
\end{itemize}
...
En este trabajo se detalló el análisis y solución del problema DCMST. Se demostró su complejidad NP-Hard, se presentaron cuatro variantes algorítmicas exactas y dos metaheurísticas, que fueron evaluadas y comparadas sobre las mismas intancias de prueba.
Los algoritmos exactos mostraron limitaciones de escalabilidad, siendo viables solo para instancias pequeñas y medianas. Entre ellos, el enfoque basado en código de Gray recursivo con poda fue el más robusto, alcanzando soluciones óptimas en instancias de hasta 11 nodos.
Las metaheurísticas demostraron ser más efectivas para instancias grandes. El método primal aleatorizado (RPM) y las colonias de hormigas (ACS) lograron soluciones cercanas al óptimo en tiempos razonables. En particular, las variantes de ACS con poblaciones ajustadas mostraron un balance favorable entre calidad de solución y tiempo de ejecución.
...
% \subsection{Solución 3: Código de Gosper hack} \label{sec:code_iter}
% \lstinputlisting[
%     language=C++
% ]{../src/solutions/iterative.cpp}

% A continuación se presentan las implementaciones de las metaheurísticas.

% \subsection{Metaheurística 1: Método Primal Aleatorizado (RPM)} \label{sec:code_rpm}
% \lstinputlisting[
% language=C++
% ]{../src/metaheuristics/evolutionary.cpp}

% \subsection{Metaheurística 2: Colonia de Hormigas (ACS)} \label{sec:code_acs}
% \lstinputlisting[
% language=C++
% ]{../src/metaheuristics/ant_colony.cpp}
